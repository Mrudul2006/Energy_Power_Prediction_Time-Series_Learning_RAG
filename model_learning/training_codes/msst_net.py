"""
MSST-Net: Multi-Scale Seasonal Transformer Network for hourly energy forecasting.

This module implements the complete architecture described in the design note:

1. Box-Cox plus fixed fractional differencing input transformation.
2. Multi-scale hourly, daily, and weekly representations.
3. Fourier plus Indian-holiday future-known calendar covariates.
4. Decomposition-aware attention with local and periodic trainable priors.
5. Dual trend/seasonal encoder streams.
6. Hierarchical cross-resolution fusion.
7. Future-known covariate decoder.
8. Point plus quantile heads and the requested loss terms.
9. Physics-informed soft constraints.
10. Temporal MinT reconciliation.
11. Split conformal calibration for quantile intervals.

The file is intentionally standalone. It does not depend on the existing
statistical analysis module, but it follows the same NumPy/Pandas/SciPy stack
and uses PyTorch for the neural components.
"""
...
class BoxCoxFractionalDifferencer:
    ...
    def fit(self, values: Sequence[float]) -> "BoxCoxFractionalDifferencer":
        ...
        transformed, weights = fractional_difference_fixed_width(
            z,
            d=d,
            threshold=self.fracdiff_threshold,
            max_size=self.max_fracdiff_weights,
        )
        self.lam = lam
        self.shift = shift
        self.d = d
...
def train_one_epoch(
    ...
    daily_total_key: Optional[str] = None,
    ...
    for batch in loader:
        ...
        outputs = model(batch)
        estimated_daily_total = batch[daily_total_key] if daily_total_key else None
        losses = loss_fn(outputs, batch["target"], estimated_daily_total=estimated_daily_total)
...
def predict(
    ...
    for batch in loader:
        batch = move_batch_to_device(batch, device)
        out = model(batch)
        points.append(out["point"].detach().cpu().numpy())
        quantiles.append(out["quantiles"].detach().cpu().numpy())
        targets.append(batch["target"].detach().cpu().numpy())
    return np.concatenate(points, axis=0), np.concatenate(quantiles, axis=0), np.concatenate(targets, axis=0)
...
def make_msst_model_from_series(
    values: Sequence[float],
    horizon: int,
    **config_overrides: object,
) -> Tuple[MSSTNet, BoxCoxFractionalDifferencer]:
    transformer = BoxCoxFractionalDifferencer().fit(values)
    transformed = transformer.transform(values)
    acf_init = estimate_acf_initializers(transformed[np.isfinite(transformed)])
    config_kwargs = {"horizon": horizon, **acf_init, **config_overrides}
    model = MSSTNet(MSSTNetConfig(**config_kwargs))
...
def _synthetic_hourly_energy(n_hours: int = 24 * 760, seed: int = 7) -> pd.DataFrame:
    ...
    temp = 26 + 8 * np.sin(2 * np.pi * doy / 365.25) + 4 * np.sin(2 * np.pi * hour / 24)
    energy = (
        520
        + 0.006 * np.arange(n_hours)
        + 85 * np.sin(2 * np.pi * hour / 24)
        + 28 * np.cos(2 * np.pi * dow / 7)
        + 35 * np.sin(2 * np.pi * doy / 365.25)
        + 3.5 * np.maximum(temp - 25, 0)
        + rng.normal(0, 16, n_hours)
    )
    return pd.DataFrame({"timestamp": ts, "energy_mwh": energy})
...
if __name__ == "__main__":
    ...
    VAL_FRAC    = 0.15   # last 15 % of origins become validation
    SAVE_PATH   = "msst_net_checkpoint.pt"

    # ── Dataset ─────────────────────────────────────────────────────────────
    print("Loading dataset …")
    frame = pd.read_csv("cleaned_energy_data.csv")
    dataset = MultiScaleEnergyDataset(
        frame,
        timestamp_col="start_time",
        target_col="electricity_consumption_(mwh)",
        horizon=HORIZON,
        stride=24,   # one origin per day keeps the run manageable
    )
    ...
    print(f"{'Epoch':>5}  {'Train loss':>12}  {'Train huber':>12}  {'Train pinball':>14}  {'Val loss':>10}  {'LR':>10}")
    print("-" * 75)

    best_val = float("inf")
    for epoch in range(1, EPOCHS + 1):
        train_metrics = train_one_epoch(model, train_loader, optimiser, loss_fn, device=DEVICE, grad_clip=1.0)
        scheduler.step()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                for k, v in vloss.items():
                    val_totals[k] = val_totals.get(k, 0.0) + float(v.detach().cpu()) * bsz
        val_metrics = {k: v / max(val_count, 1) for k, v in val_totals.items()}

        current_lr = scheduler.get_last_lr()[0]
        print(
            f"{epoch:>5}  "
            f"{train_metrics['loss']:>12.2f}  "
            f"{train_metrics['huber']:>12.2f}  "
            f"{train_metrics['pinball']:>14.2f}  "
        ...
        if val_metrics["loss"] < best_val:
            ...
            torch.save({"epoch": epoch, "model_state": model.state_dict(), "config": config}, SAVE_PATH)

    print(f"\nTraining complete. Best val loss: {best_val:.2f}  →  saved to {SAVE_PATH}")
